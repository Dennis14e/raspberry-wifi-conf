...
import { wifi_manager } from "./app/wifi_manager";
import { dependency_manager } from "./app/dependency_manager";
import { access_point } from "./config.json";
...
series([
    ...
    function test_is_wifi_enabled(next_step) {
        wifi_manager.is_wifi_enabled(function(error, result_ip) {
            ...
            if (result_ip) {
                console.log("\nWifi is enabled.");
                var reconfigure = access_point.force_reconfigure || false;
                if (reconfigure) {
    ...
    function start_http_server(next_step) {
        console.log("\nHTTP server running...");
        require("./app/api.js").default(wifi_manager, next_step);
    },
